...
local function main()
  ...
  local encoder_embeddings, decoder_embeddings, feat_embeddings
  ...
  encoder:apply(function(m)
      if torch.type(m) == "onmt.FeaturesEmbedding" then
        local tables = m.net:findModules("nn.LookupTable")
        for i = 1, #dicts.src.features do
          print("Found source features embeddings #" .. i .. " of size " ..  tables[i].weight:size(1))
          if tables[i].weight:size(1) == checkpoint.dicts.src.features[i]:size() then
            table.insert(feat_embeddings, tables[i].weight)
          end
        end
        return
      end
  ...
  print("Writing source feature embeddings")
  for i = 1, #dicts.src.features do
    write_embeddings(opt.output_dir .. "/src_feat_embeddings_".. i ..".txt", dicts.src.features[i], feat_embeddings[i])
  end
